...
export default async function SelectExercisePage({
  ...
  const [exercises, total, trainingPacks] = await Promise.all([
    ...
    prisma.trainingPack.findUnique({
      where: {
        id: packId,
      },
      select: {
  ...
  const trainingPackExercises = trainingPacks?.exercises ?? [];
  ...
  return (
    <div>
      ...
      <section className="py-4">
        ...
        {trainingPackExercises.length === 0 && (
          <p>No exercises in this plan !</p>
        )}
        {trainingPackExercises.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-2">
            {trainingPackExercises.map((exercise) => (
              <span
                key={exercise.id}
